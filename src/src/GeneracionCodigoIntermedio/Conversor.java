...
public class Conversor {
    ...
    public static Terceto getTercetoConversion(String operacion, String lexema1, String lexema2){
        ...
        if (lexema1.contains("[")){
            ...
            tipo1 = Tabla_Simbolos.getAtributos(lexema1).getTipo();//double
        }
        if (lexema2.contains("[")){
            tipo2 = GeneradorCod.getTipoTerceto(GeneradorCod.getIndexActual());//ulong
        } else{
            tipo2 = Tabla_Simbolos.getAtributos(lexema2).getTipo();
        }
        int fila = getIndex(tipo1); //2
        int col = getIndex(tipo2);  //1
        String elemento = "";
        if (operacion.equals("a")){
            //matriz de asignaciones
    ...
    private static Terceto generarTerceto(String tipo1, String lexema1, String tipo2, String lexema2, String elemento){
        ...
        operandoConvertido = operandoConvertir;//2
        String tipoResultante = elemento.substring(1); //--> a que convierte    //double
